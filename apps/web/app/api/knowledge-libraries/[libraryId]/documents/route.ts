...
import { ensureKnowledgeLibraryDirectoryPath } from "@/lib/api/workspace-directories";
...
export async function POST(
  ...
  await ensureKnowledgeLibraryDirectoryPath(
    {
      libraryId,
      workspaceId: null,
      path: directoryPath,
  ...
  const [existingDocument] = await db
    .select()
    .from(documents)
    .where(and(eq(documents.libraryId, libraryId), eq(documents.logicalPath, logicalPath)))
    .limit(1);
