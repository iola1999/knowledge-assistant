...
### 20.1 P0: Web / BFF Main Flow Hardening

目标：

- 先把“注册登录 -> 工作空间 -> 上传资料 -> 查看处理状态 -> 创建对话 -> 阅读文档 -> 回访报告”这条传统产品主链路做顺。
...
- 新用户登录后无需手动调 API 就能发起第一轮对话。
- 点击引用后能在文档页看到足够的原文上下文，而不是只有元数据。
- 上传失败、处理中、成功三种状态在界面上可感知。
...
### 20.3 P0.6: Retrieval Provider Hardening
